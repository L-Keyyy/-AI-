...
const ICONS = {
  ...
  gripVertical:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><circle cx="9" cy="6" r="1.2"/><circle cx="9" cy="12" r="1.2"/><circle cx="9" cy="18" r="1.2"/><circle cx="15" cy="6" r="1.2"/><circle cx="15" cy="12" r="1.2"/><circle cx="15" cy="18" r="1.2"/></svg>',
...
style.textContent = `
  ...
  * { box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; }
  ...
  .resize-handle-grip svg { width: 10px; height: 44px; stroke-width: 1.8; }
  .resize-handle:hover .resize-handle-grip,
  .sidebar.is-resizing .resize-handle-grip {
    color: var(--text-main);
    opacity: 1;
  ...
  .history-refresh-btn:hover { background: var(--hover-color); color: var(--text-main); }
  .history-list {
    overflow-y: auto;
    padding: 8px;
    display: flex;
  ...
  /* Messages */
  .message-wrapper { display: flex; flex-direction: column; gap: 8px; width: 100%; min-width: 0; max-width: 100%; }
  .message { font-size: 14px; line-height: 1.6; white-space: pre-wrap; word-wrap: break-word; overflow-wrap: anywhere; color: var(--text-main); }
  ...
  .message-ai { padding: 4px 0; align-self: stretch; width: 100%; max-width: 100%; display: flex; gap: 12px; }
  .message-ai-avatar { width: 24px; height: 24px; flex-shrink: 0; margin-top: 2px; }
  .message-ai-avatar svg { width: 100%; height: 100%; }
  .message-ai-content { flex: 1; min-width: 0; width: 100%; white-space: normal; }
...
container.innerHTML = `
  ...
  <div class="sidebar" id="sidebar">
    <div class="resize-handle" id="resize-handle" title="拖动调整侧边栏宽度">
      <div class="resize-handle-grip">${ICONS.gripVertical}</div>
    </div>
    ...
    <div class="header">
      ...
      <div class="header-actions">
        ...
        <button class="icon-btn" id="header-menu-btn" title="聊天记录">${ICONS.panel}</button>
    ...
    <div class="history-panel" id="history-panel">
      <div class="history-panel-header">
        <span id="history-panel-title">聊天记录</span>
        <button class="history-refresh-btn" id="history-refresh-btn" type="button">刷新</button>
      </div>
      <div class="history-list" id="history-list"></div>
      <div class="history-empty" id="history-empty">点击“刷新”加载聊天记录</div>
    </div>
...
const historyPanelTitle = shadow.getElementById('history-panel-title');
const historyRefreshBtn = shadow.getElementById('history-refresh-btn');
const historyList = shadow.getElementById('history-list');
const historyEmpty = shadow.getElementById('history-empty');
...
const NOTES_EXPORT_FOLDER = '笔记';
const HISTORY_CACHE_TTL_MS = 30 * 1000;

const historyCacheByProvider = {
  gemini: [],
  doubao: [],
};
const historyLoadedAtByProvider = {
  gemini: 0,
  doubao: 0,
};
const historyErrorByProvider = {
  gemini: '',
  doubao: '',
};
let historyLoadingProvider = '';
...
function normalizeHistoryItems(items) {
  ...
  return items
    .map((item, index) => {
      const title = normalizeText(item && (item.title || item.name || item.text || ''));
      const url = item && item.url ? String(item.url) : '';
      const id = item && item.id ? String(item.id) : url || `history-${index}`;
...
function renderHistoryPanel() {
  ...
  const items = historyCacheByProvider[provider] || [];
  const isLoading = historyLoadingProvider === provider;
  const errorMessage = historyErrorByProvider[provider] || '';

  historyPanelTitle.textContent = `${providerLabel} 聊天记录`;
  historyList.innerHTML = '';

  if (isLoading) {
    historyEmpty.textContent = `正在加载 ${providerLabel} 聊天记录...`;
    historyEmpty.style.display = '';
    return;
  }

  if (errorMessage) {
    historyEmpty.textContent = `加载失败：${errorMessage}`;
    historyEmpty.style.display = '';
    return;
  }

  if (!items.length) {
    historyEmpty.textContent = `暂无 ${providerLabel} 聊天记录（可点刷新重试）`;
    historyEmpty.style.display = '';
    return;
  }
  ...
  items.forEach((item) => {
    ...
    button.dataset.chatTitle = item.title;
    button.dataset.chatUrl = item.url || '';
...
async function loadHistoryForCurrentProvider(forceRefresh = false) {
  const provider = currentProvider;
  const now = Date.now();
  const hasFreshCache =
    !forceRefresh &&
    historyCacheByProvider[provider].length > 0 &&
    now - historyLoadedAtByProvider[provider] < HISTORY_CACHE_TTL_MS;

  if (hasFreshCache) {
    historyErrorByProvider[provider] = '';
    renderHistoryPanel();
    return;
  }

  historyLoadingProvider = provider;
  historyErrorByProvider[provider] = '';
  renderHistoryPanel();

  try {
    const response = await requestGeminiAction('gemini_get_history', { provider });
    historyCacheByProvider[provider] = normalizeHistoryItems(response.items);
    historyLoadedAtByProvider[provider] = Date.now();
  } catch (error) {
    historyErrorByProvider[provider] = error.message || '读取聊天记录失败';
  } finally {
    if (historyLoadingProvider === provider) {
...
function toggleHistoryPanel(forceOpen) {
  const nextOpen = typeof forceOpen === 'boolean' ? forceOpen : !historyPanel.classList.contains('open');
  setHistoryPanelOpen(nextOpen);
  if (!nextOpen) return;

  toggleModelMenu(false);
  toggleSettingsPanel(false);
  void loadHistoryForCurrentProvider(false);
}

async function openHistoryChatItem(item) {
  if (!item || !item.id) return;

  try {
    const response = await requestGeminiAction('gemini_open_chat', {
    ...
    if (!response || response.success === false) {
      throw new Error((response && response.message) || '打开聊天记录失败。');
    }
    setHistoryPanelOpen(false);
    clearConversation(true);
  } catch (error) {
    addMessage(`Error: ${error.message || '打开聊天记录失败。'}`, 'ai');
  }
}
...
function normalizeMessageText(text) {
  ...
  const filteredLines = value
    ...
    .filter((line) => {
      ...
      return !transientLinePatterns.some((pattern) => pattern.test(trimmed));
    });
...
function restoreStructuredBreaks(text) {
  let value = normalizeMessageText(text);
  if (!value) return '';

  const lineCount = value.split('\n').length;
  const isFlattenedLongText = value.length >= 140 && lineCount <= 3;
  if (!isFlattenedLongText) {
    return value;
  }

  const rules = [
    // "：一、" / "。二、" / "；三、"
    { re: /([。！？；;:：])\s*(?=([一二三四五六七八九十]+[、.．]))/gu, replace: '$1\n\n' },
    // "：1." / "。2."
    { re: /([。！？；;:：])\s*(?=(\d+\.\s*))/g, replace: '$1\n\n' },
    // "：•"
    { re: /([。！？；;:：])\s*(?=(?:[-*•]\s+))/gu, replace: '$1\n' },
    // break before section heading token even without punctuation
    { re: /\s+([一二三四五六七八九十]+[、.．]\s*)/gu, replace: '\n\n$1' },
    // break before numbered item token
    { re: /\s+(\d+\.\s+)/g, replace: '\n$1' },
    // break before bullet token
    { re: /\s+([-*•]\s+)/gu, replace: '\n$1' },
    // common label-style segments
    {
      re: /\s+(?=(?:核心结论|结论|总结|建议|逻辑|执行逻辑|操作路径|系统指令示例|效益|下一步|风险|说明)[:：])/gu,
      replace: '\n',
    },
  ];

  rules.forEach(({ re, replace }) => {
...
function renderInlineMarkdown(text) {
  ...
  html = html.replace(
    ...
    (match, prefix, url) => {
      ...
      return `${prefix}<a href="${href}" target="_blank" rel="noreferrer">${url}</a>`;
    }
  );
  html = html.replace(/`([^`\n]+)`/g, '<code>$1</code>');
  html = html.replace(/~~([^~\n]+)~~/g, '<del>$1</del>');
...
function markdownToHtml(markdown) {
  const source = restoreStructuredBreaks(markdown);
  if (!source) return '';
  ...
  lines.forEach((line) => {
    ...
    const cjkSectionHeadingMatch = line.match(/^\s*([一二三四五六七八九十]+[、.．])\s*(.+)$/u);
    if (cjkSectionHeadingMatch) {
      flushParagraph();
      flushQuote();
      flushList();
      html.push(`<h3>${renderInlineMarkdown(`${cjkSectionHeadingMatch[1]} ${cjkSectionHeadingMatch[2]}`)}</h3>`);
      return;
    }
...
function refreshUIState() {
  ...
  chatInput.placeholder = isNoteMode ? '输入内容并保存为 Markdown 笔记...' : '使用 AI 处理当前页面任务...';
...
async function handleNewChat() {
  clearConversation(true);
  try {
    await requestGeminiAction('gemini_new_chat', { provider: currentProvider });
    addMessage(`已在 ${getProviderLabel(currentProvider)} 中新建对话。`, 'ai');
  } catch (error) {
...
function buildNoteMarkdown(note) {
  const normalizedText = normalizeMessageText(note && note.text ? note.text : '');
  const date = new Date(Number(note && note.createdAt) || Date.now()).toLocaleString('zh-CN', { hour12: false });
  const title = String((note && note.title) || document.title || '未命名笔记').trim() || '未命名笔记';
  const url = String((note && note.url) || location.href || '').trim();
  const body = normalizedText || '（空笔记）';
...
function exportNoteMarkdown(note) {
  const createdAt = Number(note && note.createdAt) || Date.now();
  const title = sanitizeFileSegment(note && note.title ? note.title : document.title);
  const filename = `${NOTES_EXPORT_FOLDER}/${formatTimestampForFile(createdAt)}-${title}.md`;

  return sendRuntimeMessage({
    action: 'save_note_markdown',
    filename,
    title,
    createdAt,
...
function saveNote(text) {
  ...
  exportNoteMarkdown(note).then((response) => {
    if (!response || response.success === false) {
      const errorMessage = response && response.message ? response.message : 'Markdown 导出失败。';
      addMessage(`Error: ${errorMessage}`, 'ai');
    }
  }).catch((error) => {
    addMessage(`Error: ${error.message || 'Markdown 导出失败。'}`, 'ai');
  });
}
...
headerMenuBtn.addEventListener('click', () => toggleHistoryPanel());
closeSettingsBtn.addEventListener('click', () => toggleSettingsPanel(false));
historyRefreshBtn.addEventListener('click', () => {
  void loadHistoryForCurrentProvider(true);
});
historyList.addEventListener('click', (event) => {
  const button = event.target && event.target.closest ? event.target.closest('.history-item') : null;
  if (!button) return;

  void openHistoryChatItem({
    id: button.dataset.chatId || '',
    title: button.dataset.chatTitle || '',
    url: button.dataset.chatUrl || '',
  });
});
...
window.addEventListener('click', (event) => {
  ...
  const clickedHistory =
    path.includes(historyPanel) ||
    path.includes(historyList) ||
    path.includes(historyRefreshBtn) ||
